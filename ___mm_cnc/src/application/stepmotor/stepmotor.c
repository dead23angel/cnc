...
static void stepm_powerOff(uint8_t id) {
	...
	switch(id) {
		case 0: //M0_EN_PORT->BSRRH = M0_EN_PIN; break;
		case 1: //M1_EN_PORT->BSRRH = M1_EN_PIN; break;
		case 2: //M2_EN_PORT->BSRRH = M2_EN_PIN; break;
		case 3: M_EN_PORT->BSRRH = M_EN_PIN; break;
	}
}
...
void stepm_init(void) {
	...
	GPIO_InitStructure.GPIO_Pin = M_EN_PIN;
	GPIO_Init(M_EN_PORT, &GPIO_InitStructure);


	GPIO_InitStructure.GPIO_Pin = M0_DIR_PIN;
	GPIO_Init(M0_DIR_PORT, &GPIO_InitStructure);

	GPIO_InitStructure.GPIO_Pin = M0_STEP_PIN;
	GPIO_Init(M0_STEP_PORT, &GPIO_InitStructure);
	stepm_powerOff(0);


	GPIO_InitStructure.GPIO_Pin = M1_DIR_PIN;
	GPIO_Init(M1_DIR_PORT, &GPIO_InitStructure);

	GPIO_InitStructure.GPIO_Pin = M1_STEP_PIN;
	GPIO_Init(M1_STEP_PORT, &GPIO_InitStructure);
	stepm_powerOff(1);


	GPIO_InitStructure.GPIO_Pin = M2_DIR_PIN;
	GPIO_Init(M2_DIR_PORT, &GPIO_InitStructure);

	GPIO_InitStructure.GPIO_Pin = M2_STEP_PIN;
	GPIO_Init(M2_STEP_PORT, &GPIO_InitStructure);
	stepm_powerOff(2);


	GPIO_InitStructure.GPIO_Pin = M3_DIR_PIN;
	GPIO_Init(M3_DIR_PORT, &GPIO_InitStructure);

	GPIO_InitStructure.GPIO_Pin = M3_STEP_PIN;
	GPIO_Init(M3_STEP_PORT, &GPIO_InitStructure);
	stepm_powerOff(3);
...
void stepm_proc(uint8_t id) {
	...
	if(step_motors[id].isInProc) {
		switch(id) {
			case 0:
				if(step_motors[id].clk) M0_STEP_PORT->BSRRL = M0_STEP_PIN;
				else M0_STEP_PORT->BSRRH = M0_STEP_PIN;
				break;
			case 1:
				if(step_motors[id].clk) M1_STEP_PORT->BSRRL = M1_STEP_PIN;
				else M1_STEP_PORT->BSRRH = M1_STEP_PIN;
				break;
			case 2:
				if(step_motors[id].clk) M2_STEP_PORT->BSRRL = M2_STEP_PIN;
				else M2_STEP_PORT->BSRRH = M2_STEP_PIN;
				break;
			case 3:
				if(step_motors[id].clk) M3_STEP_PORT->BSRRL = M3_STEP_PIN;
				else M3_STEP_PORT->BSRRH = M3_STEP_PIN;
				break;
		}
		...
		if(steps_buf_sz > 0 && !step_motors[0].isInProc && !step_motors[1].isInProc && !step_motors[2].isInProc && !step_motors[3].isInProc) {
			...
			if(step_motors[2].steps) {
	 		...
	 		if(isEncoderCorrection && step_motors[2].steps > ENCODER_CORRECTION_MIN_STEPS) {
	 			...
	 			TIM_SetAutoreload(TIM4, p->arrValue[2]);
			}
			...
			if(step_motors[3].steps) {
				...
				TIM_SetAutoreload(TIM5, p->arrValue[3]);
			}
