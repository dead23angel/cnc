...
inline int32_t chk_Speed(int32_t i, uint32_t fxyze[], uint32_t abs_dxyze[]) __attribute__ ((always_inline));
inline int8_t findInAccelerationCrd(MVECTOR *p, MVECTOR *p_in, int32_t *smothdF, int32_t *frqStart) __attribute__ ((always_inline));
inline int8_t findOutAccelerationCrd(MVECTOR *p, MVECTOR *p_out, int32_t *smothdF, int32_t *frqStop) __attribute__ ((always_inline));
...
void cnc_gfile(char *fileName, int mode) {
	...
	do {
		...
		} // end for(p = fileBuf; !isGcodeStop && *p != 0; lineNum++, p += *p+1)
	...
	if((curGCodeMode & GFILE_MODE_MASK_EXEC) == 0) {
		...
		scr_fontColor(Green,Black);
		scr_gotoxy(1,0);
		scr_printf("Time %02d:%02d:%02d(%02d:%02d:%02d) N.cmd:%d", t1/3600,(t1/60)%60, t1%60, t2/3600,(t2/60)%60, t2%60, lineNum);
...
} // end cnc_gfile(char *fileName, int mode)
  ...
  for(i = 0; i < 3; i++) { prevF[i]=fxyze[i];  prevDir[i] = dir_xyze[i];}
	...
	if(isPause) {
		...
		while(isPause) {
			switch(kbd_getKey()) {
				case KEY_C:
					isPause = FALSE; // fix, added 031113
					return FALSE;
				case KEY_B:
					isPause = FALSE;
			}
...
inline int8_t findInAccelerationCrd(MVECTOR *p, MVECTOR *p_in, int32_t *smothdF, int32_t *frqStart) {
	...
	for(i = 0; i < 3; i++) {
		if(p->frq[i] == 0 || df[i] < _smParam.smoothStartF_from0[i]) { dt[i] = 0; continue; } // движения по координате - нет. значит торможение ускорение отсутвует.
	...
	for(i = 0; i < 3; i++) {
		if(max_time < dt[i]) { // ищем ось с максимальным временем ускорения
			crd = (int8_t)i;
			max_time = dt[i];
		}
	}
...
inline int8_t findOutAccelerationCrd(MVECTOR *p, MVECTOR *p_out, int32_t *smothdF, int32_t *frqStop) {
	...
	for(i = 0; i < 3; i++) {
		...
		if(p->frq[i] == 0 || frq_out[i] > (int32_t)p->frq[i]) {
			dt[i] = 0;
			continue;
		}
...
uint8_t smothLine(int32_t dx, int32_t dy, int32_t dz, int32_t de, int32_t time_msec, 
  ...
  DBG("\nl.next dx:%c%d(%d)\tdy:%c%d(%d)\tdz:%c%d(%d) cos_a:%f", p_next->dir[0]?'+':'-', p_next->steps[0], p_next->frq[0],
	...
	DBG("\ncrd_in:%d df_in:%d frq_in:%d  crd_out:%d df_out:%d frq_out:%d",
		crd_in, df_in, frq_in, crd_out, df_out, frq_out);
	...
	if(crd_in < 0 && crd_out < 0) { // линия целиком без необходимости ускорения/торможения
		...
		for(i = 0; TRUE; i++) {// идем от конца тормозного пути
			...
			if(crd_in >= 0) {
				...
				if((dd_f1 < 0 && f1 > max_f) || (dd_f1 > 0 && f1 < max_f)) {
					f1+=dd_f1;
					s_in += f1*SM_SMOOTH_TFEED/K_FRQ/1000; // интегрирование по разгонному участку
				}
			}
...
uint8_t cnc_line(double x, double y, double z, double extruder_length, double moveLength, double feed_rate) {
	int32_t newX = lround(x*SM_X_STEPS_PER_360/MM_PER_360), newY = lround(y*SM_Y_STEPS_PER_360/MM_PER_360);
	int32_t newZ = lround(z*SM_Z_STEPS_PER_360/MM_PER_360), newE = lround(extruder_length*SM_E_STEPS_PER_MM);
