/**
  ...
  *          GPIO) are defined in sdio_sd_conf.h file, and the initialization is 
  *          performed in SD_LowLevel_Init() function declared in sdio_sd_conf.c 
  *          file.
  ...
  *                  "SDIO_INIT_CLK_DIV" define inside the sdio_sd_conf.h file. 
  ...
  *                  define inside the sdio_sd_conf.h file.
  *                  The SD Card frequency (SDIO_CK) is computed as follows:
  *
  *                     +---------------------------------------------+
  *                     | SDIO_CK = SDIOCLK / (SDIO_INIT_CLK_DIV + 2) |
...
/** @defgroup SDIO_SD_Private_Function_Prototypes
  * @{
  */
...
uint8_t SD_Detect(void)
{
  ...
  if (GPIO_ReadInputDataBit(SD_CD_PORT, SD_CD_PIN) != Bit_RESET)
  {
