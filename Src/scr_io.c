...
static int8_t cur_col = 0, cur_row = 0, cur_width = 320/FONT_STEP_X, cur_height = 240/FONT_STEP_Y;
static uint8_t flagScroll = true;
...
void scr_waitKey(uint16_t tkey) {
	char tmp_cnt = 0;
	while(kbd_getKey() != tkey){
		if(tmp_cnt < 4){
			tmp_cnt++;
			scr_puts(".");
			_delay_ms(300);
			...
			tmp_cnt = 0;
			scr_gotoxy(scr_getCurCol()-4, scr_getCurRow());
			scr_puts("    ");
			scr_gotoxy(scr_getCurCol()-4, scr_getCurRow());
			_delay_ms(300);
		}
