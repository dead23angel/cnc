...
void GUI_Text(unsigned short x, unsigned short y, const unsigned char *str,
	...
	for (i = 0; i < len; i++) {
		ili9328_PutChar((x + 8 * i), y, *str++, Color, bkColor);
	}
}
...
void GUI_Circle(unsigned short cx, unsigned short cy, unsigned short r,
	...
	while (y > x) {
		if (fill) {
			...
			ili9328_SetPoint(cx + x, cy + y, color);
			ili9328_SetPoint(cx - x, cy + y, color);
			ili9328_SetPoint(cx + x, cy - y, color);
			ili9328_SetPoint(cx - x, cy - y, color);
			ili9328_SetPoint(cx + y, cy + x, color);
			ili9328_SetPoint(cx - y, cy + x, color);
			ili9328_SetPoint(cx + y, cy - x, color);
			ili9328_SetPoint(cx - y, cy - x, color);
		}
...
void GUI_Rectangle(unsigned short x0, unsigned short y0, unsigned short x1,
	...
	if (fill) {
		for (h++; x0 <= x1; x0++)
			ili9328_VLine(x0, y0, h, color);
		return;
	}
	ili9328_VLine(x0, y0, h, color);
	GUI_Line(x0, y0, x1, y0, color);
	GUI_Line(x0, y1, x1, y1, color);
	ili9328_VLine(x1, y0, h, color);
}
