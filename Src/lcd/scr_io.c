...
static int8_t cur_col = 0, cur_row = 0, cur_width = 320 / FONT_STEP_X,
		cur_height = 240 / FONT_STEP_Y;
static uint8_t flagScroll = 1;
static uint16_t cur_x_ofs = 0, cur_y_ofs = 0;

void scr_setfullTextWindow() {
	cur_x_ofs = 0;
	cur_y_ofs = 0;
	cur_width = 320 / FONT_STEP_X;
	cur_height = 240 / FONT_STEP_Y;
}
...
void scr_setTextWindow(uint16_t x, uint16_t y, uint8_t maxCol, uint8_t maxRow) {
	...
	cur_y_ofs = y;
	cur_width =
			((maxCol * FONT_STEP_X + x) >= 320) ?
					(320 - x) / FONT_STEP_X : maxCol;
	cur_height =
			((maxRow * FONT_STEP_Y + y) >= 240) ?
					(240 - y) / FONT_STEP_Y : maxRow;
}
...
void scr_putc(char c) {
	...
	case '\r':
		break;
	default:
		if (cur_col >= 0 && cur_col < cur_width && cur_row >= 0
				&& cur_row < cur_height)
			ili9328_PutChar((uint16_t) cur_col * FONT_STEP_X + cur_x_ofs,
					(uint16_t) cur_row * FONT_STEP_Y + cur_y_ofs, c, fontColor,
					fontBkColor);
		cur_col++;
	}
}
...
//-------------------------------------------------
static void win_showWindow(uint16_t x0, uint16_t y0, uint8_t colSz,
		uint8_t rowSz, uint16_t cBorder, uint16_t cFill, uint16_t cText) {
	uint16_t x1 = x0 + (uint16_t) colSz * FONT_STEP_X + 10;
	uint16_t y1 = y0 + (uint16_t) rowSz * FONT_STEP_Y + 10;
	GUI_Rectangle(x0, y0, x1, y1, cBorder, 0);
	GUI_Rectangle(x0 + 1, y0 + 1, x1 - 1, y1 - 1, cBorder, 0);
	GUI_Rectangle(x0 + 2, y0 + 2, x1 - 2, y1 - 2, cBorder, 0);
	GUI_Rectangle(x0 + 3, y0 + 3, x1 - 3, y1 - 3, cFill, 1);
	scr_setTextWindow(x0 + 5, y0 + 5, (x1 - x0 - 10) / FONT_STEP_X,
			(y1 - y0 - 10) / FONT_STEP_Y);
	scr_fontColor(cText, cFill);
}

void win_showErrorWin(void) {
	win_showWindow(FONT_STEP_X * 4 - 5, FONT_STEP_Y * 4 - 5, 32, 7, Yellow, Red,
	Black);
	scr_gotoxy(0, 0);
	scr_setScrollOn(1);
}
...
void win_showMsgWin(void) {
	win_showWindow(FONT_STEP_X * 4 - 5, FONT_STEP_Y * 4 - 5, 32, 8, Magenta,
	Blue2, Cyan);
	scr_gotoxy(0, 0);
	scr_setScrollOn(1);
}

void win_showProgressBar(uint16_t x, uint16_t y, uint16_t dx, uint16_t dy,
		uint8_t p) {
	uint16_t n = (dx) * p / 100;
	GUI_Rectangle(x, y, x + dx, y + dy, Red, 0);
	GUI_Rectangle(x + 1, y + 1, x + dx - 1, y + dy - 1, Red, 0);
	GUI_Rectangle(x + 2 + n - 1, y + 2, x + dx - 2, y + dy - 2, Black, 1);
	GUI_Rectangle(x + 2, y + 2, x + n + 1, y + dy - 2, Yellow, 1);
}
...
void win_showMenuScroll(uint16_t x, uint16_t y, uint8_t col, uint8_t row,
	...
	GUI_Rectangle(x1, y, x1 + 7, y1, White, 1);
	GUI_Rectangle(x1 - 1, y + ofs + 2, x1 + 4, y + ofs + n, Blue2, 1);
