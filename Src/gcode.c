...
#define NEXT_ACTION_CW_ARC 7
#define NEXT_ACTION_CCW_ARC 8
#define NEXT_ACTION_SPINDLE_ON  21
#define NEXT_ACTION_SPINDLE_OFF 22
#define NEXT_ACTION_COOLANT_ON  23
#define NEXT_ACTION_COOLANT_OFF 24
...
//if (line[*char_counter] == 0 || line[*char_counter] == ';' || line[*char_counter] == '\n' || line[*char_counter] == '\r' ) return FALSE;
 if (line[*char_counter] == 0 || line[*char_counter] == ';' || line[*char_counter] == '%' || line[*char_counter] == '(' || line[*char_counter] == '\n' || line[*char_counter] == '\r' ) return FALSE;
 *letter = line[*char_counter];
 //if((*letter < 'A') || (*letter > 'Z')) {
 	//gc.status_code = GCSTATUS_EXPECTED_COMMAND_LETTER; //TODO
  //return FALSE;
// }
 (*char_counter)++;
...
uint8_t gc_execute_line(char *line) {
	...
	if(line[0] == ';' || line[0] == '(' || line[0] == '%') { return GCSTATUS_OK; } // comments
	// Pass 1: Commands
			...
			case 'g':
				switch(int_value) {
					case 0: // Rapid linear move
						...
						break;
					case 1: // Linear move at programed feed rate
					...
					case 49: // Tool length offset compensation cancel
						break;
					case 18: // Select X-Z plane
					case 19: // Select Y-Z plane
					case 41: // Cutter compensation, left of programmed path
					case 42: // Cutter compensation, right of programmed path
					case 43: // Tool length offset compensation
					case 53: // Machine coordinate system
					case 54: // Coordinate system 1
					case 55: // Coordinate system 2
					case 56: // Coordinate system 3
					case 57: // Coordinate system 4
					case 58: // Coordinate system 5
					case 59: // Coordinate system 6
			...
			case 'm':
				switch(int_value) {
					...
					case 3:
						gc.spindle_dir = 0;
						gc.spindle_on = 1;
						gc.next_action = NEXT_ACTION_SPINDLE_ON;
						break;
					case 4:
						gc.spindle_dir = 1;
						gc.spindle_on = 1;
					...
					case 5:
						gc.spindle_on = 0;
						gc.next_action = NEXT_ACTION_SPINDLE_OFF;
						break;
					case 7:// Coolant mist on
					case 8:// Coolant flood on
						gc.next_action = NEXT_ACTION_COOLANT_ON;
						break;
					case 9:// Coolant off
						gc.next_action = NEXT_ACTION_COOLANT_OFF;
						break;
					case 23: // Thread gradual pullout ON
					case 24: // Thread gradual pullout OFF
					case 50: // M50 P0 (off) or P1 (on) Feed Override Control
					case 51: // M51 P0 (off) or P1 (on) Spindle Speed Override Control
	...
	}//end while(next_statement(&letter, &value, line, &char_counter))
	...
	switch(gc.next_action) {
		...
		case NEXT_ACTION_CCW_ARC:
			if(radius_mode) {
				 ...
				 */
				double h_x2_div_d = -sqrt(4 * radius*radius - dx*dx - dy*dy)/hypot(dx,dy); // == -(h * 2 / d)
				// If r is smaller than d, the arc is now traversing the complex plane beyond the reach of any
				// real CNC, and thus - for practical reasons - we will terminate promptly:
				if(isnan(h_x2_div_d)) FAIL(GCSTATUS_FLOATING_POINT_ERROR);
				// Invert the sign of h_x2_div_d if the circle is counter clockwise (see sketch below)
				if(gc.next_action == NEXT_ACTION_CCW_ARC) { h_x2_div_d = -h_x2_div_d; }
				/*
            	...
            	will have > 180 deg of angular travel      |          < 180 deg of angular travel, which is a good thing!
                                             	 \         |          /
				center of arc when h_x2_div_d ->  x <----- | -----> x <- center of arc when h_x2_div_d is negative
								is positive                |
							                           	   |
                                                       	   |
                                                       	   C  <-- Current position
			...
			} else { // end if(radius_mode)
				//Offset mode specific computations
				radius = hypot(offset[X_AXIS], offset[Y_AXIS]); // Compute arc radius for mc_arc
			} // end else if(radius_mode)

			mc_arc(oldPosition, gc.position, offset, gc.feed_rate, radius, gc.next_action == NEXT_ACTION_CW_ARC);
			break;
		case NEXT_ACTION_SPINDLE_ON:
			spindle_on(gc.s_value, gc.spindle_dir);
			break;
		case NEXT_ACTION_SPINDLE_OFF:
			spindle_off();
			break;
		case NEXT_ACTION_COOLANT_ON:
			coolant_on();
			break;
		case NEXT_ACTION_COOLANT_OFF:
			coolant_off();
			break;
	} // end switch(gc.next_action)

	return(gc.status_code);

} // end gc_execute_line(char *line)
